...
def _warn_default_admin():
    ...
    try:
        ...
        print(f"[WARN] default admin password check failed: {e}")
...
""",
        ...
        print("[Sentinel] 知识库种子数据已注入 (6 篇安全指南)")
...
def create_app():
    ...
    print("[DEBUG] calling init_db()...", flush=True)
    init_db()
    print("[DEBUG] init_db() done", flush=True)
    _warn_default_admin()

    # Phase 4: 定时扫描调度迁移
    print("[DEBUG] calling migrate_scheduler...", flush=True)
    conn4 = get_db()
    from services.scheduler_service import migrate_scheduler
    migrate_scheduler(conn4)
    conn4.close()
    print("[DEBUG] migrate_scheduler() done", flush=True)

    print("[DEBUG] registering blueprints...", flush=True)
    ...
    try:
        ...
        init_scheduler()
        print("[Sentinel] Scan scheduler started")
    except Exception as e:
        print(f"[Sentinel] Scheduler init skipped: {e}")

    print("=" * 56)
    print("  Sentinel AppSec Platform v4.0 (Phase 1: Security Hardened)")
    print(f"  API:     http://{FLASK_HOST}:{FLASK_PORT}/api/")
    print(f"  Frontend: http://localhost:{FLASK_PORT}/")
    print(f"  Scanner:  {SCANNER_MODE}")
    print(f"  Email:    {'enabled' if SMTP_CONFIG['enabled'] else 'disabled (set SMTP env vars to enable)'}")
    print("=" * 56)
...
def _serve_frontend(app: Flask):
    ...
    if not os.path.isdir(FRONTEND_DIST):
        print(f"[Sentinel] FRONTEND_DIST not found at {FRONTEND_DIST}")
        print("[Sentinel] 前端未构建，请先执行: cd frontend && npm run build")
        print("[Sentinel] 或使用开发模式: cd frontend && npm run dev")
        return
    ...
    print(f"[Sentinel] Serving frontend from {FRONTEND_DIST}")
